...
class PadesSignatureStarter extends SignatureStarter {
	...
	setPdfToSignFromContentBase64(contentBase64) {
		this._pdfToSign = FileReference.fromContent(Buffer.from(contentBase64, 'base64'));
	}
	...
	async _startCommonGetRequest() {
		...
		try {
			...
			if (apiVersion >= 2) {
				...
				request.pdfToSign = Buffer.from(await this._pdfToSign.getContent()).toString('base64');
			}
