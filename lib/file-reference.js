	...
	async uploadOrReference(client) {
		try {
			if (this._blobToken) {
				...
				return new FileModel({content: Buffer.from(this._content).toString('base64')});
				...
				return this._openOrUseExistingStream(async s => {
					if (s.readableLength < client.multipartUploadThreshold) {
						...
						return new FileModel({content: Buffer.from(buf).toString('base64')});
					} else {
						let uploadResult = await client._uploadOrRead(s);
						if ((typeof uploadResult) == 'string') {
							return new FileModel({blobToken: uploadResult});
						} else {
							return new FileModel({content: Buffer.from(uploadResult).toString('base64')});
						}
					}
				});
			}
