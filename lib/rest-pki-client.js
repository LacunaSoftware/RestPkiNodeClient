...
class RestPkiClient {
	...
	async _upload(stream) {
		...
		try {
			...
			if(endRequest.partETags == 0){
				throw new Error("partETags = 0, uploadParts not completed succesfully");
			}
	...
	async _uploadParts(stream, client, blobUri, partSize) {
			...
			await this.waitToBeReadable(stream);
			...
			if (this._multipartUploadDoubleCheck) {
				...
				return({'partETags': partETags});
			}
	}

	waitToBeReadable(stream) {
