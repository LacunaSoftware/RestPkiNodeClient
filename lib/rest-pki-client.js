...
class RestPkiClient {
	...
	static initializeRestPkiVersion(endpointUrl) {
		if (!this._restPkiVersion) {
			// Lazy initialization - retrieve and set the static property once
			const client = new RestPkiClient(endpointUrl); // Create a temporary instance to call instance methods
			RestPkiClient._restPkiVersion = client._tryGetEndpointVersion(endpointUrl);
		}
		return RestPkiClient._restPkiVersion;
	}

	getRestPkiVersion() {
		if (!RestPkiClient._restPkiVersion) {
			// Initialize if not already set
			RestPkiClient.initializeRestPkiVersion(this._endpointUrl);
		}
		return RestPkiClient._restPkiVersion;
	}
	...
	getRestPkiVersion() {
		if (!this._restPkiVersion) {
			this._restPkiVersion = this._tryGetEndpointVersion(this._endpointUrl);
		} else {
		...
		return new Promise((resolve) => {
			...
			stream.on("readable", async () => {
				...
				if (this._multipartUploadDoubleCheck) {
					resolve({
						'partETags': partETags,
						'completeMD5': Buffer.from(hasher.digest()).toString('base64')
					});
				} else {
