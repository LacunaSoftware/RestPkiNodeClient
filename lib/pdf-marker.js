...
class PdfMarker {
	...
	setFileFromContentBase64(contentBase64) {
		this._fileContent = Buffer.from(contentBase64, 'base64');
	}
	...
	async apply() {
		...
		request.file = {
			content: Buffer.from(this._fileContent).toString('base64')
		};
		try {
			let response = await this._client.getRestClient().post('Api/Pdf/AddMarks', request);
			return new FileResult(this._client, response.file);
